...
export async function GET(req: Request) {
  ...
  // Do not rely on Supabase's default 1,000-row response limit. A chapter
  // can contain more than 1,000 questions, and the UI must show the real
  // total/attempted numbers and allow every question to be practiced.
  const questions: any[] = [];
  const pageSize = 1000;
  ...
  for (let from = 0; from < ids.length; from += progressBatchSize) {
    const batch = ids.slice(from, from + progressBatchSize);
    const { data, error: progressError } = await db.from('qb_question_progress')
