...
export async function POST(req: Request) {
  try {
    ...
    const { testId, answers, timeSpentSeconds, questionTimeSeconds, markedForReview, bookmarked } = body;
    ...
    const { data: attempt, error } = await db
      ...
      .insert({
        ...
        answers: { ...(answers || {}), __markedForReview: markedForReview || {}, __bookmarked: bookmarked || {} },
...
// This endpoint only accepts POST for submitting an examination.
// Keeping an explicit GET handler also prevents Next.js route-type generation
// from confusing this route with the dynamic /api/test-series/[id] route.
export async function GET(_req: Request) {
  return NextResponse.json(
    { error: 'Method not allowed. Use POST to submit an examination.' },
