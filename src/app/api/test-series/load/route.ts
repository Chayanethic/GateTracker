...
export async function GET(req: Request) {
  try {
    ...
    if (!id || id === 'undefined' || id === 'null') {
      return NextResponse.json({ error: 'A valid test id is required.' }, { status: 400 });
    }
    ...
    if (!test) return NextResponse.json({ error: 'Test not found or no longer published.' }, { status: 404 });

    return NextResponse.json({ test }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || 'Unable to load examination.' }, { status: 500 });
  }
}
