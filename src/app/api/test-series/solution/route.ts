...
export async function GET(req: Request) {
  try {
    ...
    const { data: key, error } = await database.from('test_series_keys').select('id_in_test,solution_html,video_url').eq('test_series_id', testId).eq('id_in_test', questionId).maybeSingle();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    if (!key) return NextResponse.json({ error: 'Question not found.' }, { status: 404 });

    return NextResponse.json({ solution: { solutionHtml: key.solution_html || '', videoUrl: key.video_url || null } });
  } catch (e: any) {
