...
import { ArrowLeft, Bookmark, PlayCircle, Search, Sun, Moon } from 'lucide-react';
...
export default function MarkedQuestionsPage() {
  ...
  return (
    <div className={bg + " h-[100dvh] overflow-y-auto overflow-x-hidden custom-scrollbar"}>
      <div className="w-full px-4 sm:px-6 lg:px-10 py-5 lg:py-8">
        ...
        <div className="mt-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-violet-500"><Bookmark size={20}/><span className="text-xs font-black uppercase tracking-[.2em]">Bookmark Bank</span></div>
            <h1 className="mt-2 text-3xl font-black">Bookmarked Questions</h1>
            <p className={light ? 'mt-2 text-slate-500' : 'mt-2 text-zinc-500'}>Questions you explicitly bookmarked during an exam. This list is separate from Mark for Review.</p>
          </div>
          ...
          {filtered.map(item => (
            <article key={item.key} className={light ? 'rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm' : 'rounded-2xl border border-white/10 bg-[#222] overflow-hidden'}>
              <div className={light ? 'px-5 py-4 border-b border-slate-200 flex items-center justify-between' : 'px-5 py-4 border-b border-white/10 flex items-center justify-between'}>
                <div><div className="font-black">Q{item.number} · {item.type}</div><div className="text-xs text-violet-500 mt-1">Bookmarked in Attempt {item.attemptNumber}</div></div>
        ...
        {!filtered.length && <div className={light ? 'mt-8 rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500' : 'mt-8 rounded-2xl border border-white/10 bg-[#222] p-12 text-center text-zinc-500'}>{items.length ? 'No matching marked questions.' : 'No bookmarked questions yet. Press Bookmark beside any question during an exam and submit the exam to save it here.'}</div>}
      </div>
    </div>
  );
}
