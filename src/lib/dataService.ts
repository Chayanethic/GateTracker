...
export const syncTopicTrackingCompletion = async (
  ...
  const finished = (completed || []).length >= ids.length;

  await supabase
    .from('topic_tracking')
    .update({ completed_at: finished ? new Date().toISOString() : null, updated_at: new Date().toISOString() })
    .eq('user_id', userId)
    .eq('subject_name', material.subject_name)
    .eq('topic_name', material.topic_name);
};
